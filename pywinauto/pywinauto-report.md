`Устаревшая версия документа. Документ перенесен в *.docx формат, так как большой объем текста из markdown в docx переносить неудобно.`

---

# Введение

Графический интерфейс пользователя (*GUI - Graphical User Interface*) - разновидность пользовательского интерфейса, в котором элементы интерфейса (меню, кнопки, значки, списки и т.п.), представленные пользователю на дисплее, исполнены в виде графических изображений.
...
На сегодняшний день существует множество различных по виду, способу работы и поддерживаемым приложениям инструментов автоматизации GUI. Некоторые из них представляют собой библиотеку для использования с каким-либо языком программирования или же отдельное приложение (программа для управления другими программами). Одни позволяют лишь сымитировать работу с клавиатурой и мышью, другие же могут распознать и выполнить действия с каждым отдельным элементом GUI приложения. Существуют как платные, так и open-source (с открытым исходным кодом) решения.

Каждый из этих инструментов поддерживает определенный набор приложений, и программист может не найти подходящего для своей задачи по автоматизации GUI open-source решения. Поэтому ему придется выходить из ситуации либо упрощая свою задачу по автоматизации, либо использовать сразу несколько доступных инструментов, разбивая задачу на части, либо рассматривать вариант использования платных проектов.
...
- Написан на скриптовом языке общего назначения (например, Python, как довольно популярном на сегодняшний день языке, ориентированным на повышение производительности разработчика и читаемости кода).
- Может быть использован в качестве подключаемой к языку библиотеки.
- Прост в обращении и имеет интуитивно понятный интерфейс.
- Поддерживает приложения нескольких видов (Native, Windows Forms, Windows Presentation Foundation).
...
**pywinauto** - набор Python модулей, позволяющих управлять GUI приложениями, посылая действия клавиатуры и мыши окнам и отдельным элементам графического интерфейса приложения.

Особенности pywinauto:

- Имеет открытый исходный код, размещенный на платформе GitHub [8].
...
Подробнее об Accessibility технологиях, подходах к автоматизации GUI, оконной системе ОС Windows в разделе ***Графический интерфейс Windows***.

Подробнее о pywinauto в разделе ***Библиотека pywinauto***.

Целями данной работы являются:

- Изучение библиотеки pywinauto, принципа ее работы, архитектуры.
- Расширение класса поддерживаемых библиотекой приложений (Windows Forms, Windows Presentation Foundation, Windows Store Applications) путем использования Accessibility технологии User Interface Automation для ОС Windows.
...
**Win32 API** – библиотека ОС Windows, набор классов, процедур, функций, структур и констант, которые используют программы для создания и работы с системными элементами в OC Windows, в том числе и с окнами.
...
- **Координатный метод.** Пользователь явно указывает, куда нужно кликнуть мышью, какие клавиши нажать на клавиатуре.
...
На операционной системе Windows существует 2 вида API, позволяющих получить программный доступ к элементам GUI:
...
- **User Interface Automation (UIA)** -  управляющий API, поддерживаемый более новыми элементами управления, который также может воспользоваться интерфейсом MSAA для получения доступа к стандартным элементам.
...
Каждый элемент пользовательского интерфейса представляется в виде объекта класса *AutomationElement*, который предоставляет методы для получения свойств элемента, поиска его дочерних элементов, генерации событий.

# Существующие инструменты автоматизации GUI
...
- Другие бесплатные инструменты:
 ...
 - Пользовательский сборник инструментов: (<https://github.com/atinfo/awesome-test-automation>).
- Платные инструменты:
...
# Библиотека pywinauto 0.5.4

Библиотека **pywinauto** - набор Python модулей, позволяющих управлять GUI приложениями, посылая действия клавиатуры и мыши окнам и отдельным элементам графического интерфейса приложения. Библиотека имеет открытый исходный код и размещена на платформе GitHub.
...
![](resources/images/appveyor.png)
**Рисунок 1.** Серсис AppVeyor.

### Анализ кода

![](resources/images/code-analysis.png)
**Рисунок 2.** Анализаторы кода.
...
8. pywinauto - Windows GUI Automation with Python. URL: <https://github.com/pywinauto/pywinauto> (дата обращения: 17.05.2016).
9. pywinauto slides by Valentin Kroupkin from Auckland Python Meetup. URL: <http://www.meetup.com/fr-FR/NZPUG-Auckland/events/225405249/> (дата обращения: 17.05.2016).
10. pywinauto - AppVeyor CI. URL: <https://ci.appveyor.com/project/pywinauto/pywinauto> (дата обращения: 17.05.2016).
11. PEP 8 -- Style Guide for Python Code. URL: <https://www.python.org/dev/peps/pep-0008/> (дата обращения 19.05.2016).
12. PEP 257 -- Docstring Conventions. URL: <https://www.python.org/dev/peps/pep-0257/> (дата обращения 19.05.2016).
